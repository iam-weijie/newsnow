...
const brewMarkets = defineSource(async () => {
  ...
  if (!result?.length) throw new Error("Cannot fetch Brew Markets data")

  return result.map((issue): NewsItem => ({
    id: issue.slug,
    title: issue.title.trim(),
    url: `https://www.brewmarkets.com/issues/${issue.slug}`,
